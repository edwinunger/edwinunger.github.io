...
<br/>
<code>
<font color = "red">Ruby Version:<br>
def separate_comma(number)<br>
  digits = []<br>
  new_number = number.to_s.reverse!<br>
  while new_number.length > 0<br>
    digits << new_number.slice!(0, 3)<br>
  end<br>
...
function separate_comma(number) {<br>
  var digits = [];<br>
  var newNumber = number.toString().split('').reverse();<br>
  while (newNumber.length > 0) {<br>
    var spliced = newNumber.splice(0, 3).join('');<br>
    digits.push(spliced);<br>
  }<br>
